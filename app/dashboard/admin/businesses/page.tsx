...
import { useAdminBusinessesStore } from "@/lib/stores/admin-business-store";
...
export default function AdminBusinessesPage() {
    ...
    /* ============================================================
        LOAD BUSINESSES & SYNC TO LOCAL STATE
    ============================================================ */
    const businessesQuery = useQuery({
        queryKey: ["admin", "businesses-directory"],
        queryFn: async () => {
            const response = await getAllMembers();
            const allMembers = response.documents as unknown as Business[];
            return allMembers.filter((member) => member.role === "business");
        },
    });
    ...
    const filteredBusinesses = useMemo(() => {
        ...
        return businesses.filter((business) => {
            const matchesSearch =
                !search ||
                business.fullName.toLowerCase().includes(search) ||
                business.userId.toLowerCase().includes(search) ||
                (business.phone || "").toLowerCase().includes(search);

            const matchesVerification =
                verificationFilter === "all" ||
                business.verificationStatus === verificationFilter;

            return matchesSearch && matchesVerification;
        });
    }, [businesses, searchTerm, verificationFilter]);
    ...
    const handleStatusChange = async (business: Business, newStatus: string) => {
        if (business.verificationStatus === newStatus) return;

        const confirmation = await Swal.fire({
            title: "Update Verification Status?",
            text: `Change ${business.fullName}'s status to ${newStatus}?`,
            icon: "question",
        ...
        try {
            ...
            await databases.updateDocument(DATABASE_ID, MEMBERS_TABLE_ID, business.$id, {
                verificationStatus: newStatus,
            });

            // Immediately update local state so table and metrics refresh instantly
            setBusinesses((prev) =>
                prev.map((b) => (b.$id === business.$id ? { ...b, verificationStatus: newStatus } : b))
            );

            Swal.fire({
                icon: "success",
                title: "Status Updated",
                text: `${business.fullName} is now ${newStatus}.`,
                timer: 1500,
        ...
        await Swal.fire({
            ...
            html: `
                <div style="text-align:left;line-height:1.8">
                    <p><strong>User ID:</strong> ${business.userId}</p>
                    <p><strong>Phone:</strong> ${business.phone || "Not provided"}</p>
                    <p><strong>Role:</strong> Business</p>
                    <p><strong>Profile Completion:</strong> ${business.profileCompletion}%</p>
                    <p><strong>Verification:</strong> ${business.verificationStatus}</p>
                    <p><strong>Created:</strong> ${formatDate(business.$createdAt)}</p>
                </div>
            `,
    ...
    const handleEditBusiness = (business: Business) => {
        router.push(`/dashboard/admin/users/edit?id=${business.$id}`);
    };
        ...
        <div className="min-h-screen bg-[#f4f7fb] text-slate-800 antialiased">
            ...
            <aside className={`fixed inset-y-0 left-0 z-50 flex w-72 flex-col bg-[#0b1a2e] text-white transition-transform duration-300 ease-in-out lg:hidden ${mobileMenuOpen ? "translate-x-0 shadow-2xl" : "-translate-x-full"}`}>
                ...
                <nav className="flex-1 space-y-1.5 overflow-y-auto px-4 py-4 text-xs font-bold">
                    ...
                    <Link href="/dashboard/admin/businesses" onClick={() => setMobileMenuOpen(false)} className="flex items-center gap-3.5 rounded-xl bg-blue-600 px-4 py-3 text-white shadow-md"><span>🏢</span><span>Businesses</span></Link>
            ...
            <aside className="fixed inset-y-0 left-0 z-40 hidden w-64 flex-col border-r border-slate-900/10 bg-[#0b1a2e] text-white lg:flex">
                ...
                <nav className="flex-1 space-y-1 overflow-y-auto px-4 py-5 text-xs font-bold [scrollbar-width:none]">
                    ...
                    <Link href="/dashboard/admin/businesses" className="flex items-center gap-3.5 rounded-xl bg-blue-600 px-4 py-3 text-white shadow-md shadow-blue-600/30"><span>🏢</span><span>Businesses</span></Link>
                ...
                <div className="flex items-center gap-4">
                    <div className="flex items-center gap-3 pl-2 border-l border-slate-200">
                        ...
                        <div className="hidden text-left sm:block">
                            ...
                            <p className="text-[10px] font-medium text-slate-400">Business Management</p>
                        </div>
                    </div>
            ...
            <main className="p-4 sm:p-6 lg:ml-64 space-y-6 max-w-7xl mx-auto">
                    ...
                    <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                        <div className="max-w-xl">
                            ...
                            <p className="mt-2 text-xs sm:text-sm text-slate-600 leading-relaxed">
                                View and manage HomeMate business accounts, verification status and account information.
                            </p>
                        </div>
                        ...
                        <div className="hidden md:flex items-center gap-4 rounded-2xl bg-white/80 p-4 border border-white shadow-sm backdrop-blur">
                            ...
                            <div>
                                <p className="text-xs font-bold text-slate-900">Vendor Operations</p>
                                <p className="text-[11px] text-slate-500">Supervise marketplace businesses and partners.</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
                    <div className="flex items-center gap-4 rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm">
                        ...
                        <div>
                            <p className="text-xs font-bold text-slate-400">Total Businesses</p>
                            <p className="text-2xl font-black text-slate-900 mt-0.5">{totalBusinesses}</p>
                            <p className="text-[10px] text-slate-400">Registered businesses</p>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-4 rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm">
                        ...
                        <div>
                            ...
                            <p className="text-[10px] text-emerald-600 font-bold">Verified businesses</p>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-4 rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm">
                        ...
                        <div>
                            ...
                            <p className="text-[10px] text-amber-600 font-bold">Awaiting review</p>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-4 rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm">
                        ...
                        <div>
                            ...
                            <p className="text-[10px] text-rose-500 font-medium">Rejected businesses</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="rounded-3xl border border-slate-200/80 bg-white shadow-sm overflow-hidden">
                    <div className="border-b border-slate-100 p-5 flex items-center justify-between">
                        <div>
                            <h3 className="text-base font-black text-slate-900">HomeMate Businesses</h3>
                            <p className="text-xs text-slate-500">Registered business accounts.</p>
                        </div>
                        <span className="text-xs font-bold text-slate-400">Showing {filteredBusinesses.length} of {businesses.length} businesses</span>
                    </div>
                    ...
                    {businessesQuery.isLoading ? (
                        <div className="flex min-h-[300px] items-center justify-center">
                            <div className="text-center">
                                ...
                                <p className="text-xs font-bold text-slate-400">Loading businesses...</p>
                            </div>
                        </div>
                    ) : filteredBusinesses.length === 0 ? (
                        <div className="p-16 text-center">
                            <span className="text-4xl">🏢</span>
                            <h3 className="text-base font-bold text-slate-900 mt-3">No Businesses Found</h3>
                            <p className="text-xs text-slate-400 mt-1">No business accounts match your current search or verification filter.</p>
                        </div>
                    ) : (
                            ...
                            <table className="w-full text-left text-xs">
                                ...
                                <tbody className="divide-y divide-slate-100 font-medium text-slate-700">
                                    {filteredBusinesses.map((business) => (
                                        <tr key={business.$id} className="hover:bg-slate-50/60 transition">
                                            <td className="p-4 pl-6 flex items-center gap-3">
                                                <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-blue-600 text-xs font-bold text-white shadow-sm">
                                                    {business.fullName.charAt(0).toUpperCase()}
                                                </div>
                                                <div>
                                                    <p className="font-bold text-slate-900">{business.fullName}</p>
                                                    <p className="text-[10px] text-slate-400">{business.$id}</p>
                                                </div>
                                            </td>
                                            <td className="p-4 font-mono text-[11px] text-slate-500">{business.userId}</td>
                                            <td className="p-4 text-slate-600">{business.phone || "Not provided"}</td>
                                            <td className="p-4">
                                                <div className="flex items-center gap-3">
                                                    <span className={`text-xs font-bold ${getProfileCompletionClass(business.profileCompletion)}`}>
                                                    ...
                                                    <div className="h-1.5 w-16 rounded-full bg-slate-100 overflow-hidden">
                                                        <div
                                                            ...
                                                            style={{
                                                                width: `${Math.min(Math.max(business.profileCompletion, 0), 100)}%`,
                                                            }}
                                                        />
                                                    </div>
                                                </div>
                                            ...
                                            <td className="p-4">
                                                <select
                                                    ...
                                                    onChange={(e) => handleStatusChange(business, e.target.value)}
                                                    className={`rounded-xl px-3 py-1.5 text-xs font-bold uppercase outline-none shadow-sm cursor-pointer ${getVerificationBadgeClass(business.verificationStatus)}`}
                                                >
                                                    <option value="Pending">Pending</option>
                                                    <option value="Approved">Approved</option>
                                                    <option value="Rejected">Rejected</option>
                                            ...
                                            <td className="p-4 text-slate-500">{formatDate(business.$createdAt)}</td>
