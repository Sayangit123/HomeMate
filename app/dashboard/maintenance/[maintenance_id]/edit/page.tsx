...
const maintenanceSchema = yup.object({
  ...
  providerName: yup
    ...
    .max(
      100,
      "Provider name must not exceed 100 characters"
    )
    .default(""),
...
export default function EditMaintenancePage() {
  ...
  // -----------------------------------------
  // Get maintenance ID from dynamic route
  // -----------------------------------------
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        const [recordResponse, propertiesResponse] =
          await Promise.all([
        ...
        const record =
          recordResponse as unknown as MaintenanceRecord;
        ...
        if (record.userId !== user.$id) {
          await Swal.fire({
            icon: "error",
            title: "Access denied",
            text: "You are not authorized to edit this maintenance record.",
          });
        ...
        // -----------------------------------------
        // Format Appwrite Date for HTML date input
        // -----------------------------------------
        ...
        await Swal.fire({
          ...
          text: "The maintenance record could not be loaded.",
        });
  ...
  const onSubmit = async (
    ...
    try {
      ...
      router.push(
        `/dashboard/maintenance/${maintenance.$id}`
      );
    } catch (error) {
      console.error(
      ...
      Swal.fire({
        icon: "error",
        title: "Update failed",
        text: "Unable to update the maintenance record. Please try again.",
      });
    } finally {
      setSubmitting(false);
    }
      ...
      <div className="mx-auto max-w-4xl">
        ...
        <div className="mb-8">
          ...
          <p className="mt-2 text-sm text-slate-400">
            Update the details of your maintenance record.
          </p>
        </div>
